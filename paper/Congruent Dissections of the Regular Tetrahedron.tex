...
Two certificate-backed reductions extend the programme.
...
\textbf{(B)}  The public $n=5$ stack has been sharpened beyond the earlier
four-hypothesis formulation.  Exact certificates discharge \textup{(H-coc)}
and \textup{(H-Qb)}, \textup{(H-trip)} is retired as a formal hypothesis,
and parity kills the singleton/odd \textup{(H-orb)} patterns.  The remaining
open case is the accidental even \textup{(H-orb)} residual: $f(P)=7$, $t=3$,
$\Sym(P)=1$, with the four interior facets accidentally congruent in an even
metric pattern $(4)$ or $(2,2)$.  Thus \Cref{thm:n5} is a reduction to this
single residual, not an unconditional $n=5$ closure.
...
\begin{conditionaltheorem}[$n=5$ convex residual reduction, preview]\label{thm:n5-intro}
...
unconditionally or by the exact public certificates discharging
\textup{(H-coc)} and \textup{(H-Qb)}.  \textup{(H-trip)} is retired, and the
singleton/odd \textup{(H-orb)} branches are killed by parity.  See
\Cref{thm:n5,sec:prog-n5} for the precise ledger.
...
reduction $f(P) \in \{5, 7\}$, weak Congruence Transport, direct H-coc and
H-Qb certificates, and H-orb parity/scope ledgers -- transfers directly to
the remaining small non-divisors.
...
\cref{thm:n5} ($n = 5$ reduction to a single H-orb residual) and
...
reduced to one \textup{(H-orb)} residual}\label{sec:prog-n5}
...
stack is sharper than the earlier four-hypothesis formulation.  The active
ledger is:
\begin{itemize}
  \item \textup{(H-trip)} is retired: it is no longer a formal input to the
        public theorem.
  \item \textup{(H-coc)} is discharged directly by the orientation-bit
        certificate \texttt{hcoc\_orientation\_certificate.py}.
  \item \textup{(H-Qb)} is discharged by the Qb certificate stack
        \texttt{hqb\_*\_certificate.py}.
  \item \textup{(H-orb)} is not discharged in blanket form.  Its singleton
        and odd-multiplicity branches are killed by parity, and its only
        remaining branch is the accidental even metric residual described in
        \Cref{thm:n5}.
\end{itemize}
Thus the correct theorem-level claim is not an unconditional impossibility of
$n=5$, but a reduction of every putative dissection to one explicitly scoped
H-orb residual.  Throughout this subsection the standing embedding
is~\eqref{eq:16-embedding}.
...
\begin{conditionaltheorem}[$n=5$ convex reduction to the accidental-even \textup{(H-orb)} residual]\label{thm:n5}
...
Equivalently, $n=5$ is reduced to the single H-orb accidental-even residual.
No unconditional impossibility of a $5$-piece convex dissection is claimed at
this stage.  The certificate ledger is:
\begin{itemize}
  \item \textup{(H-trip)} is retired and is not used as a formal hypothesis;
  \item \textup{(H-coc)} is closed by
        \texttt{hcoc\_orientation\_certificate.py}, which fixes the
        orientation bit in the $K_{5}$ triangle system;
  \item \textup{(H-Qb)} is closed by
        ...
        \texttt{hqb\_opp\_certificate.py};
  \item the singleton and odd-multiplicity \textup{(H-orb)} branches are
        closed by \texttt{horb\_parity\_certificate.py};
  \item \texttt{horb\_residual\_scope.py} and
        \texttt{horb\_residual\_combinatorial.py} identify the remaining
...
``$n=5$ reduced to a single H-orb accidental even residual.''
\end{conditionaltheorem}
...
ledgers that isolate the single accidental-even H-orb branch.
...
\begin{remark}[Current status of \textup{(H-orb)}]\label{rem:h-orb-auto}
The blanket orbit-detection statement \textup{(H-orb)} is no longer used as an
undischarged standing hypothesis.  The weak Congruence Transport Lemma gives
metric equality of paired facets.  The certificate
\texttt{horb\_parity\_certificate.py} then kills every odd-multiplicity
interior-facet pattern, including the singleton patterns that earlier drafts
handled through \textup{(H-orb)}.  The only surviving patterns are the even
ones, $(4)$ and $(2,2)$.  When these are genuine $\Sym(P)$-orbits, the
orientation certificate and \Cref{lem:k5-cyc} close the branch.  What remains
is precisely the accidental case with $\Sym(P)=1$, where equal facet metrics
do not come from a symmetry of $P$; this is the open H-orb residual recorded
in \texttt{horb\_residual\_scope.py}.
\end{remark}
...
\begin{remark}[Historical \textup{(H-trip)} route to \textup{(H-coc)}]\label{rem:hcoc-from-htrip}
Earlier drafts recorded a developing-map route intended to derive
\textup{(H-coc)} from the topological statement \textup{(H-trip)}.  That route
is no longer an active dependency of \Cref{thm:n5}: the orientation-bit
certificate \texttt{hcoc\_orientation\_certificate.py} discharges
\textup{(H-coc)} directly.  We retain the label \textup{(H-trip)} only as
historical motivation for why the cocycle condition was isolated; it is
retired from the formal public ledger.
\end{remark}

\begin{remark}[Current logical chain]\label{rem:htrip-status}
The active $f=7$ chain no longer depends on four standing hypotheses.  The
orientation-bit certificate fixes the H-coc sign obstruction directly; H-trip
is retired.  Parity closes the singleton and odd H-orb branches.  The genuine
$(2,2)$ orbit branch is closed by the H-coc orientation obstruction, while
\texttt{horb\_residual\_scope.py} records the one branch not closed by these
soft tools: accidental even metric congruence with $\Sym(P)=1$.  The synthesis
result file \path{phase26e_synthesis_theorem_26_1_results.json} records this
split via \path{closed_modules}, \path{retired_modules}, and
\path{remaining_modules}.
\end{remark}
...
\textsf{Qb-opp}~-- as the explicit geometric bridge \textup{(H-Qb)}.
...
\begin{itemize}
  ...
  \item \emph{Pyramid with its quadrilateral base on $\partial T$
        ...
        versions treated their exclusion as the standing hypothesis
        \textup{(H-Qb)}.  The present public stack discharges this branch by
...
\begin{remark}[Status of \textup{(H-Qb)}]\label{rem:hqb-status}
The \textsf{Qb-adj} and \textsf{Qb-opp} families are non-empty at the
single-piece level, so the old single-piece degeneracy argument remains
retracted.  Their exclusion is now supplied by the multi-script H-Qb
certificate stack listed in \Cref{thm:n5}.  Consequently \textup{(H-Qb)} is
not an open standing hypothesis in the current public ledger; it is a closed
module whose local decomposition is still visible in
\texttt{phase26c\_t3\_f5\_exclusion.py}.
\end{remark}
...
graph is $K_{5}$.  The old analysis closed this branch under a blanket
\textup{(H-orb)} assumption plus \textup{(H-coc)}.  The current ledger is more
precise:
\begin{itemize}
  \item odd or singleton interior-facet multiplicity patterns are impossible
        by the parity certificate, without invoking blanket \textup{(H-orb)};
  \item genuine $(2,2)$ orbit transport is closed by the direct H-coc
        orientation certificate together with \Cref{lem:k5-cyc};
  \item the remaining case is accidental even metric congruence with
        $\Sym(P)=1$, in pattern $(4)$ or $(2,2)$.
\end{itemize}
The legacy script \path{phase26c_t3_f7_exclusion.py} records the older
conditional closure under blanket orbit-detection assumptions.  The active
public synthesis is instead \path{phase26e_synthesis_theorem_26_1.py}
...
the $f=5$ branch is closed by the H-Qb certificates, and the $f=7$ branch is
closed except for the accidental even H-orb residual with $\Sym(P)=1$.
...
H-orb accidental-even residual stated in \Cref{thm:n5}.  This proves the
...
much narrower target than the earlier four-hypothesis stack.
\end{remark}
...
\begin{itemize}
  ...
  \item and the $n = 5$ convex reduction to one H-orb accidental-even
        residual (\cref{thm:n5}), the first non-divisor of $24$ reduced in the
        convex category to a single named open branch.  The current public
        stack discharges \textup{(H-coc)} and \textup{(H-Qb)}, retires
        \textup{(H-trip)}, and kills H-orb singleton/odd patterns by parity;
        the remaining task is the metric accidental-even branch with
        $f(P)=7$, $t=3$, and $\Sym(P)=1$.
\end{itemize}
